...
class GifRaw:
    ...
    def __init__(self, gif_file: str, verbose: bool = False) -> None:
        """
    ...
    def get_color_table_entry(self, image_body:LocalImageDescriptor) -> tuple:
        if image_body.has_color_table:
            ...
            scope = ColorTableType.GLOBAL
            color_tables = self.global_color_table.entries
        return scope, color_tables

    def get_transparent_color(self, index: int = -1) -> list:
        using_transparent_color = False
        color_index = self.logical_screen_desc.bg_color_index
        rgb = [None, None, None]
        cur_image_idx = 0
        for block in self.blocks:
            if block.is_extention_block:
                extention = block.get_extention
                if extention.is_graphic_control:
                    gcb = extention.get_graphic_control
                    if gcb.transparent_color_flag:
                        using_transparent_color = True
                        color_index = gcb.transparent_idx
    ...
    def get_frame_delay(self, index: int = -1) -> int:
        """
        This method is need for frame delay control.
        index is for later implement. I'm lazy.
        """
        for block in self.blocks:
            if block.is_extention_block:
                extention = block.get_extention
                if extention.is_graphic_control:
                    gcb = extention.get_graphic_control
                    if gcb.delay_time:
                        return gcb.delay_time
        """
        almost animated GIF's frame delay is 10. maybe.
        """
        return 10

    def get_fps(self, index: int = -1) -> int:
        """
        This method is need for frame delay control. especially for imageio.
        """
        delay_time = self.get_frame_delay(index)

        return 100 / delay_time
